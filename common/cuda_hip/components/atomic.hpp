...
{
    auto addr = reinterpret_cast<gpuComplex*>(address);
    // Separate to real part and imag part
    auto real = atomic_add(reinterpret_cast<float*>(&(addr->x)), val.real());
    auto imag = atomic_add(reinterpret_cast<float*>(&(addr->y)), val.imag());
    return {real, imag};
}
...
{
    auto addr = reinterpret_cast<gpuDoubleComplex*>(address);
    // Separate to real part and imag part
    auto real = atomic_add(reinterpret_cast<double*>(&(addr->x)), val.real());
    auto imag = atomic_add(reinterpret_cast<double*>(&(addr->y)), val.imag());
    return {real, imag};
}
