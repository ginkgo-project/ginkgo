...
struct elimination_forest_algorithm_state {
    ...
    IndexType* basecase_ranges() { return workspace.get_pointer(15); }

    IndexType* bucketsort_workspace() { return workspace.get_pointer(16); }

    array<IndexType> bucketsort_workspace_view()
    {
        return workspace.get_view(16);
    }
    ...
    void run()
    {
        ...
        reset_connected_components();
        for (int level = basecase_level; level < num_levels; level++) {
