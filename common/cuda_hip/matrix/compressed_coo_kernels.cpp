...
__device__ void spmv_kernel(const size_type nnz, const size_type num_lines,
                            ...
                            const uint32* __restrict__ row_bits,
                            const IndexType* __restrict__ row_ranks,
                            ...
                            const size_type c_stride, Closure scale)
{
    ...
    IndexType curr_row = (ind < nnz) ? row_bv.rank(ind) : 0;
    const auto tile_block =
        group::tiled_partition<subwarp_size>(group::this_thread_block());
    for (; ind < ind_end; ind += subwarp_size) {
        temp_val += (ind < nnz) ? val[ind] * b[col[ind] * b_stride + column_id]
                                : zero<ValueType>();
        auto next_row = (ind + subwarp_size < nnz)
                            ? row_bv.rank(ind + subwarp_size)
                            : row_bv.rank(nnz - 1);
        // segmented scan
...
__global__ __launch_bounds__(spmv_block_size) void abstract_spmv(
    ...
    const uint32* __restrict__ row_bits,
    const IndexType* __restrict__ row_ranks, const ValueType* __restrict__ b,
    const size_type b_stride, ValueType* __restrict__ c,
    const size_type c_stride)
{
    spmv_kernel(nnz, num_lines, val, col, row_bits, row_ranks, b, b_stride, c,
...
__global__ __launch_bounds__(spmv_block_size) void abstract_spmv(
    ...
    const IndexType* __restrict__ col, const uint32* __restrict__ row_bits,
    const IndexType* __restrict__ row_ranks, const ValueType* __restrict__ b,
    const size_type b_stride, ValueType* __restrict__ c,
    const size_type c_stride)
{
    ...
    spmv_kernel(
        nnz, num_lines, val, col, row_bits, row_ranks, b, b_stride, c, c_stride,
                            ...
                            const size_type c_stride, Closure scale)
{
    ...
    if (column_id < num_cols && coo_idx < nnz) {
        auto curr_row = row_bv.rank(coo_idx);
        auto idx = coo_idx;
        for (; idx < coo_end - 1; idx++) {
            temp += val[idx] * b[col[idx] * b_stride + column_id];
            const auto next_row = row_bv.rank(idx + 1);
...
__global__ __launch_bounds__(spmv_block_size) void abstract_spmm(
    ...
    const ValueType* __restrict__ val, const IndexType* __restrict__ col,
    const uint32* __restrict__ row_bits,
    const IndexType* __restrict__ row_ranks, const size_type num_cols,
    const ValueType* __restrict__ b, const size_type b_stride,
    ValueType* __restrict__ c, const size_type c_stride)
{
    spmm_kernel(nnz, num_elems, val, col, row_bits, row_ranks, num_cols, b,
          ...
          const matrix::Dense<ValueType>* b, matrix::Dense<ValueType>* c)
{
    ...
    const auto nwarps = coo::host_kernel::calculate_nwarps(exec, nnz);
        ...
        if (nwarps > 0 && b_ncols > 0) {
            ...
            if (b_ncols < 4) {
                ...
                abstract_spmv<<<coo_grid, coo_block, 0, exec->get_stream()>>>(
                    ...
                    a->get_const_col_idxs(), a->get_const_row_bits(),
                    a->get_const_row_bit_ranks(),
                ...
                abstract_spmm<<<coo_grid, coo_block, 0, exec->get_stream()>>>(
                    ...
                    a->get_const_col_idxs(), a->get_const_row_bits(),
                    a->get_const_row_bit_ranks(), b_ncols,
                    as_device_type(b->get_const_values()), b->get_stride(),
                    as_device_type(c->get_values()), c->get_stride());
            }
        }
