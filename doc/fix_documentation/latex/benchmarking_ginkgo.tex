...
\end{DoxyEnumerate}\hypertarget{benchmarking_ginkgo_autotoc_md13}{}\doxysubsection{2\+: Using $<$tt$>$ssget$<$/tt$>$ to fetch the matrices}\label{benchmarking_ginkgo_autotoc_md13}
...
or specified on the fly, on the same line as the `make benchmark'\textquotesingle{} command\+:
...
\item {\ttfamily E\+X\+E\+C\+U\+T\+OR=\{reference,cuda,hip,omp\}} -\/ select the executor and platform the benchmarks should be ran on.
...
\item {\ttfamily M\+A\+T\+R\+I\+X\+\_\+\+L\+I\+S\+T\+\_\+\+F\+I\+LE=/path/to/matrix\+\_\+list.file} -\/ allows to list Suite\+Sparse matrix id or name to benchmark. As an example, a matrix list file containing the following will ensure that benchmarks are ran for only those three matrices\+: \`{}\`{}\`{} 1903 Freescale/circuit5M thermal2 \`{}\`{}\`{}
...
\end{DoxyItemize}
\item {\ttfamily E\+X\+E\+C\+U\+T\+OR=\{reference,cuda,hip,omp\}} -\/ select the executor and platform the benchmarks should be ran on. Default is {\ttfamily cuda}.
