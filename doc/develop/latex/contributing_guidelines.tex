We are glad that you are interested in contributing to Ginkgo. Please have a look at our coding guidelines before proposing a pull request.\hypertarget{contributing_guidelines_autotoc_md12}{}\doxysection{Table of Contents}\label{contributing_guidelines_autotoc_md12}
...
\end{DoxyItemize}\hypertarget{contributing_guidelines_autotoc_md13}{}\doxysection{Most important stuff (\+A T\+L;\+D\+R)}\label{contributing_guidelines_autotoc_md13}
...
You can refer to \href{https://chris.beams.io/posts/git-commit/}{\texttt{ this informative guide}} for more details.\hypertarget{contributing_guidelines_autotoc_md20}{}\doxysubsubsection{Attributing credit}\label{contributing_guidelines_autotoc_md20}
...
In the Ginkgo commit history, this is most common associated with suggested improvements from code reviews.\hypertarget{contributing_guidelines_autotoc_md21}{}\doxysubsection{Creating, Reviewing and Merging Pull Requests}\label{contributing_guidelines_autotoc_md21}
...
\end{DoxyItemize}\hypertarget{contributing_guidelines_autotoc_md22}{}\doxysection{Code style}\label{contributing_guidelines_autotoc_md22}
\hypertarget{contributing_guidelines_autotoc_md23}{}\doxysubsection{Automatic code formatting}\label{contributing_guidelines_autotoc_md23}
...
{\itshape Note}\+: To learn more about how Clang\+Format will format your code, see existing files in Ginkgo, {\ttfamily .clang-\/format} configuration file shipped with Ginkgo, and Clang\+Format\textquotesingle{}s documentation.\hypertarget{contributing_guidelines_autotoc_md24}{}\doxysubsection{Naming scheme}\label{contributing_guidelines_autotoc_md24}
\hypertarget{contributing_guidelines_autotoc_md25}{}\doxysubsubsection{Filenames}\label{contributing_guidelines_autotoc_md25}
...
{\itshape Note\+:} A C++ source/header file is considered a {\ttfamily C\+U\+DA} file if it contains C\+U\+DA code that is not guarded with {\ttfamily \#if} guards that disable this code in non-\/\+C\+U\+DA compilers. I.\+e. if a file can be compiled by a general C++ compiler, it is not considered a C\+U\+DA file.\hypertarget{contributing_guidelines_autotoc_md26}{}\doxysubsubsection{Macros}\label{contributing_guidelines_autotoc_md26}
Macros (both object-\/like and function-\/like macros) use {\ttfamily C\+A\+P\+I\+T\+A\+L\+\_\+\+C\+A\+SE}. They have to start with {\ttfamily G\+K\+O\+\_\+} to avoid name clashes (even if they are {\ttfamily \#undef}-\/ed in the same file!).\hypertarget{contributing_guidelines_autotoc_md27}{}\doxysubsubsection{Variables}\label{contributing_guidelines_autotoc_md27}
Variables use {\ttfamily snake\+\_\+case}.\hypertarget{contributing_guidelines_autotoc_md28}{}\doxysubsubsection{Constants}\label{contributing_guidelines_autotoc_md28}
Constants use {\ttfamily snake\+\_\+case}.\hypertarget{contributing_guidelines_autotoc_md29}{}\doxysubsubsection{Functions}\label{contributing_guidelines_autotoc_md29}
Functions use {\ttfamily snake\+\_\+case}.\hypertarget{contributing_guidelines_autotoc_md30}{}\doxysubsubsection{Structures and classes}\label{contributing_guidelines_autotoc_md30}
...
All other structures and classes use {\ttfamily Camel\+Case}.\hypertarget{contributing_guidelines_autotoc_md31}{}\doxysubsubsection{Members}\label{contributing_guidelines_autotoc_md31}
...
Additionally, non-\/public data members end with an underscore ({\ttfamily \+\_\+}).\hypertarget{contributing_guidelines_autotoc_md32}{}\doxysubsubsection{Namespaces}\label{contributing_guidelines_autotoc_md32}
Namespaces use {\ttfamily snake\+\_\+case}.\hypertarget{contributing_guidelines_autotoc_md33}{}\doxysubsubsection{Template parameters}\label{contributing_guidelines_autotoc_md33}
...
\end{DoxyItemize}\hypertarget{contributing_guidelines_autotoc_md34}{}\doxysubsection{Whitespace}\label{contributing_guidelines_autotoc_md34}
...
\end{DoxyEnumerate}\hypertarget{contributing_guidelines_autotoc_md35}{}\doxysubsection{Include statement grouping}\label{contributing_guidelines_autotoc_md35}
...
\end{DoxyCode}
\hypertarget{contributing_guidelines_autotoc_md36}{}\doxysubsubsection{Main header}\label{contributing_guidelines_autotoc_md36}
...
{\itshape Note}\+: Please see the detail in the {\ttfamily dev\+\_\+tools/scripts/config}.\hypertarget{contributing_guidelines_autotoc_md37}{}\doxysubsubsection{Some general comments.}\label{contributing_guidelines_autotoc_md37}
...
\end{DoxyEnumerate}\hypertarget{contributing_guidelines_autotoc_md38}{}\doxysubsubsection{Automatic header arrangement}\label{contributing_guidelines_autotoc_md38}
...
\end{DoxyEnumerate}\hypertarget{contributing_guidelines_autotoc_md39}{}\doxysubsection{Other Code Formatting not handled by Clang\+Format}\label{contributing_guidelines_autotoc_md39}
\hypertarget{contributing_guidelines_autotoc_md40}{}\doxysubsubsection{Control flow constructs}\label{contributing_guidelines_autotoc_md40}
Single line statements should be avoided in all cases. Use of brackets is mandatory for all control flow constructs (e.\+g. {\ttfamily if}, {\ttfamily for}, {\ttfamily while}, ...).\hypertarget{contributing_guidelines_autotoc_md41}{}\doxysubsubsection{Variable declarations}\label{contributing_guidelines_autotoc_md41}
...
For this reason, {\bfseries{always}} declare each variable on a separate line, with its own {\itshape type-\/specifier}.\hypertarget{contributing_guidelines_autotoc_md42}{}\doxysubsection{C\+Make coding style}\label{contributing_guidelines_autotoc_md42}
\hypertarget{contributing_guidelines_autotoc_md43}{}\doxysubsubsection{Whitespaces}\label{contributing_guidelines_autotoc_md43}
All alignment in C\+Make files should use four spaces.\hypertarget{contributing_guidelines_autotoc_md44}{}\doxysubsubsection{Use of macros vs functions}\label{contributing_guidelines_autotoc_md44}
Macros in C\+Make do not have a scope. This means that any variable set in this macro will be available to the whole project. In contrast, functions in C\+Make have local scope and therefore all set variables are local only. In general, wrap all piece of algorithms using temporary variables in a function and use macros to propagate variables to the whole project.\hypertarget{contributing_guidelines_autotoc_md45}{}\doxysubsubsection{Naming style}\label{contributing_guidelines_autotoc_md45}
All Ginkgo specific variables should be prefixed with a {\ttfamily G\+I\+N\+K\+G\+O\+\_\+} and all functions by {\ttfamily ginkgo\+\_\+}.\hypertarget{contributing_guidelines_autotoc_md46}{}\doxysection{Helper scripts}\label{contributing_guidelines_autotoc_md46}
To facilitate easy development within Ginkgo and to encourage coders and scientists who do not want get bogged down by the details of the Ginkgo library, but rather focus on writing the algorithms and the kernels, Ginkgo provides the developers with a few helper scripts.\hypertarget{contributing_guidelines_autotoc_md47}{}\doxysubsection{Create a new algorithm}\label{contributing_guidelines_autotoc_md47}
...
\end{DoxyItemize}\hypertarget{contributing_guidelines_autotoc_md51}{}\doxysubsection{Some general rules.}\label{contributing_guidelines_autotoc_md51}
...
\end{DoxyItemize}\hypertarget{contributing_guidelines_autotoc_md52}{}\doxysubsection{Writing tests for kernels}\label{contributing_guidelines_autotoc_md52}
...
\end{DoxyItemize}\hypertarget{contributing_guidelines_autotoc_md53}{}\doxysection{Documentation style}\label{contributing_guidelines_autotoc_md53}
Documentation uses standard Doxygen.\hypertarget{contributing_guidelines_autotoc_md54}{}\doxysubsection{Developer targeted notes}\label{contributing_guidelines_autotoc_md54}
Make use of {\ttfamily @internal} doxygen tag. This can be used for any comment which is not intended for users, but is useful to better understand a piece of code.\hypertarget{contributing_guidelines_autotoc_md55}{}\doxysubsection{Whitespaces}\label{contributing_guidelines_autotoc_md55}
\hypertarget{contributing_guidelines_autotoc_md56}{}\doxysubsubsection{After named tags such as $<$tt$>$@param foo$<$/tt$>$}\label{contributing_guidelines_autotoc_md56}
The documentation tags which use an additional name should be followed by two spaces in order to better distinguish the text from the doxygen tag. It is also possible to use a line break instead.\hypertarget{contributing_guidelines_autotoc_md57}{}\doxysubsection{Documenting examples}\label{contributing_guidelines_autotoc_md57}
...
\end{DoxyEnumerate}\hypertarget{contributing_guidelines_autotoc_md58}{}\doxysection{Other programming comments}\label{contributing_guidelines_autotoc_md58}
\hypertarget{contributing_guidelines_autotoc_md59}{}\doxysubsection{C++ standard stream objects}\label{contributing_guidelines_autotoc_md59}
These are global objects and are shared inside the same translation unit. Therefore, whenever its state or formatting is changed (e.\+g. using {\ttfamily std\+::hex} or floating point formatting) inside library code, make sure to restore the state before returning the control to the user. See this \href{https://stackoverflow.com/questions/2273330/restore-the-state-of-stdcout-after-manipulating-it}{\texttt{ stackoverflow question}} for examples on how to do it correctly. This is extremely important for header files.\hypertarget{contributing_guidelines_autotoc_md60}{}\doxysubsection{Warnings}\label{contributing_guidelines_autotoc_md60}
By default, the {\ttfamily -\/D\+G\+I\+N\+K\+G\+O\+\_\+\+C\+O\+M\+P\+I\+L\+E\+R\+\_\+\+F\+L\+A\+GS} is set to {\ttfamily -\/Wpedantic} and hence pedantic warnings are emitted by default. Some of these warnings are false positives and a complete list of the resolved warnings and their solutions is listed in \href{https://github.com/ginkgo-project/ginkgo/issues/174}{\texttt{ Issue 174}}. Specifically, when macros are being used, we have the issue of having {\ttfamily extra ;} warnings, which is resolved by adding a {\ttfamily static\+\_\+assert()}. The CI system additionally also has a step where it compiles for pedantic warnings to be errors.\hypertarget{contributing_guidelines_autotoc_md61}{}\doxysubsection{Avoiding circular dependencies}\label{contributing_guidelines_autotoc_md61}
