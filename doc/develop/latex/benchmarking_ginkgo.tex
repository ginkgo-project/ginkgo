...
\item {\ttfamily P\+R\+E\+C\+O\+N\+DS=\{jacobi,ilu,parict,parilu,parilut,ilu-\/isai,parict-\/isai,parilu-\/isai,parilut-\/isai,none\}} the preconditioners to use for either {\ttfamily solver} or {\ttfamily preconditioner} benchmarks. Multiple options can be passed to this variable. Default is {\ttfamily none}.
...
\item {\ttfamily S\+O\+L\+V\+E\+RS=\{bicgstab,bicg,cg,cgs,fcg,gmres,lower\+\_\+trs,upper\+\_\+trs\}} -\/ the solvers which should be benchmarked. Multiple options can be passed. The default is {\ttfamily cg}. Note that {\ttfamily lower/upper\+\_\+trs} by default don\textquotesingle{}t use a preconditioner, as they are by default exact direct solvers.
...
\item {\ttfamily S\+O\+L\+V\+E\+R\+S\+\_\+\+R\+HS=\{unit, random\}} -\/ whether to use a vector of all ones or random values as the right-\/hand side in solver benchmarks. Default is {\ttfamily unit}.
