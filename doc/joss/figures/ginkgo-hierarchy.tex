...
drop shadow, text centered, anchor=north, text=black, text width=3.5cm]
...
\begin{tikzpicture}[node distance=1cm,
  ...
  header/.style = {%
    ...
    \node (combinedf) [abstract, rectangle, below=of resnormredf]
        {
            \textbf{Combined::Factory}
        };
    \begin{pgfonlayer}{background}
    \node[namespace=gkoyellow!70, header=stop, fit=(resnormredf) (iterationf) (combinedf) (timef)] {};
    \end{pgfonlayer}
    % solvers
    \node (bicgstabf) [abstract, rectangle, right=of AuxNode03, xshift=-1cm]
        {
    ...
    \begin{pgfonlayer}{background}
    \node[namespace=gkoorange, header=solver, fit=(bicgstabf) (cgf) (cgsf) (fcgf) (gmresf) (irf) (ltrsf) (utrsf)] {};
    \end{pgfonlayer}
    % preconditioners
    \node (jacobif) [abstract, rectangle, below=1.5cm of combinedf]
        {
            \textbf{Jacobi::Factory}
        };
        \node (pariluf) [abstract, rectangle, below=of jacobif]
        {
          \textbf{ParIlu::Factory}
        };
        \node (parilutf) [abstract, rectangle, below=of pariluf]
        {
          \textbf{ParIlut::Factory}
        };
    \begin{pgfonlayer}{background}
    \node[namespace=gkocomplbrown, header=preconditioner, fit=(jacobif) (pariluf) (parilutf)] {};
    \end{pgfonlayer}
    ...
    \node (ell) [abstract, rectangle, below=of dense]
        {
            \textbf{Ell}
        };
    ...
    \begin{pgfonlayer}{background}
    \node[namespace=gkocomplblue, header=matrix, fit=(coo) (csr) (dense) (ell) (hybrid) (identity) (permutation) (sellp) (sparsitycsr)] {};
    \end{pgfonlayer}
    % solvers
    \node (bicgstab) [abstract, rectangle, right=of AuxNode02, xshift=-1cm]
        {
            \textbf{Bicgstab}
        };
...
+ remove_logger()
    ...
    \begin{pgfonlayer}{background}
    \node[namespace=gkoorange, header=solver, fit=(bicgstab) (cg) (cgs) (fcg) (gmres) (utrs) (ltrs) (ir)] {};
    \end{pgfonlayer}
    % preconditioners
    \node (jacobi) [abstract, rectangle, below=1.5cm of sparsitycsr]
        {
            \textbf{Jacobi}
        };
    \node (parilu) [abstract, rectangle, below=of jacobi]
        {
          \textbf{ParIlu}
        };
    ...
    \begin{pgfonlayer}{background}
    \node[namespace=gkocomplbrown, header=preconditioner, fit=(jacobi) (parilu) (parilut)] {};
    \end{pgfonlayer}
    ...
    \node (time) [abstract, rectangle split, rectangle split parts=2, below=of resnormred]
        {
            \textbf{Time}
            \nodepart{second}
            {\scalefont{0.9}
...
+ remove_logger()
    ...
    \begin{pgfonlayer}{background}
    \node[namespace=gkoyellow!70, header=stop, fit=(criterion) (resnormred) (iteration) (combined) (time)] {};
    \end{pgfonlayer}
