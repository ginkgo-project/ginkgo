...
{
    return range<
        scaled_reduced_row_major<dim, cuda_type_t<Type1>, cuda_type_t<Type2>,
                                 mask, IndexType, SizeType>>(
        r.get_accessor().get_size(),
        as_cuda_type(r.get_accessor().get_stored_data()),
...
{
    return range<block_col_major<cuda_type_t<T>, dim, IndexType, SizeType>>(
