                                            ...
                                            bool skip_sorting)
{
    ...
    if (parameters_.max_block_size == 1) {
        ...
        auto diag_vt = run<matrix::Diagonal<ValueType>,
                           matrix::Diagonal<previous_precision<ValueType>>,
                           matrix::Diagonal<previous_precision<ValueType, 2>>,
                           matrix::Diagonal<previous_precision<ValueType, 3>>>(
            diag.get(), [](auto diag_c) {
                return temporary_conversion<
                    matrix::Diagonal<ValueType>>::create(diag_c);
            });
        if (!diag_vt) {
