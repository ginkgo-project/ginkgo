...
TYPED_TEST(Fcg, RecognizesInvalidSystemMatrix)
{
    using value_type = typename TestFixture::value_type;
    using Mtx = typename TestFixture::Mtx;
    using value_type = typename TestFixture::value_type;
    ...
    ASSERT_THROW(this->fcg_factory->generate(m)->validate_data(),
                 gko::InvalidData);
}


TYPED_TEST(Fcg, RecognizesInvalidPreconditioner)
{
    using value_type = typename TestFixture::value_type;
    using Mtx = typename TestFixture::Mtx;
    using Solver = typename TestFixture::Solver;
    ...
    std::shared_ptr<Solver> invalid_preconditioner =
        this->fcg_factory->generate(invalid_mtx);
    auto factory = Solver::build()
