...
TYPED_TEST(Cgs, RecognizesInvalidSystemMatrix)
{
    using value_type = typename TestFixture::value_type;
    using Mtx = typename TestFixture::Mtx;
    using value_type = typename TestFixture::value_type;
    ...
    ASSERT_THROW(this->cgs_factory->generate(m)->validate_data(),
                 gko::InvalidData);
}


TYPED_TEST(Cgs, RecognizesInvalidPreconditioner)
{
    using value_type = typename TestFixture::value_type;
    using Mtx = typename TestFixture::Mtx;
    using Solver = typename TestFixture::Solver;
    ...
    std::shared_ptr<Solver> invalid_preconditioner =
        this->cgs_factory->generate(invalid_mtx);
    auto factory =
        Solver::build()
            .with_criteria(gko::stop::Iteration::build().with_max_iters(3u))
