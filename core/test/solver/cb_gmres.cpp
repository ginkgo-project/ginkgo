...
TYPED_TEST(CbGmres, RecognizesInvalidSystemMatrix)
{
    using value_type = typename TestFixture::value_type;
    using Mtx = typename TestFixture::Mtx;
    using value_type = typename TestFixture::value_type;
    ...
    ASSERT_THROW(this->cb_gmres_factory->generate(m)->validate_data(),
                 gko::InvalidData);
}


TYPED_TEST(CbGmres, RecognizesInvalidPreconditioner)
{
    using value_type = typename TestFixture::value_type;
    using Mtx = typename TestFixture::Mtx;
    using Solver = typename TestFixture::Solver;
    ...
    std::shared_ptr<Solver> invalid_preconditioner =
        this->cb_gmres_factory->generate(invalid_mtx);
    auto factory = Solver::build()
