...
class CountSketchCore : public ::testing::Test {
protected:
    using value_type =
    ...
    CountSketchCore() : exec(gko::ReferenceExecutor::create()) {}

    std::shared_ptr<const gko::Executor> exec;
};

TYPED_TEST_SUITE(CountSketchCore, gko::test::ValueIndexTypes,
                 PairTypenameNameGenerator);


TYPED_TEST(CountSketchCore, ReturnsCorrectSketchSize)
{
...
TYPED_TEST(CountSketchCore, ReturnsCorrectInputSize)
{
    auto sketch = TestFixture::Sketch::create(this->exec, 7, 20, 42);
...
TYPED_TEST(CountSketchCore, ReturnsCorrectSeed)
{
...
TYPED_TEST(CountSketchCore, HashMapHasCorrectSize)
{
    auto sketch = TestFixture::Sketch::create(this->exec, 7, 20, 42);

    EXPECT_EQ(sketch->get_hash_map().get_size(), 20);
}


TYPED_TEST(CountSketchCore, SignsHasCorrectSize)
{
    auto sketch = TestFixture::Sketch::create(this->exec, 7, 20, 42);
...
TYPED_TEST(CountSketchCore, ApplyThrowsOnDimensionMismatch)
{
...
TYPED_TEST(CountSketchCore, ApplyThrowsOnOutputDimensionMismatch)
{
    using Dense = typename TestFixture::Dense;
    auto sketch = TestFixture::Sketch::create(this->exec, 3, 5, 42);
...
TYPED_TEST(CountSketchCore, ApplyThrowsOnColumnMismatch)
{
    using Dense = typename TestFixture::Dense;
    auto sketch = TestFixture::Sketch::create(this->exec, 3, 5, 42);
    auto b = Dense::create(this->exec, gko::dim<2>{5, 3});
...
TYPED_TEST(CountSketchCore, RapplyThrowsOnDimensionMismatch)
{
    using Dense = typename TestFixture::Dense;
    auto sketch = TestFixture::Sketch::create(this->exec, 3, 5, 42);
...
TYPED_TEST(CountSketchCore, RapplyThrowsOnOutputColumnMismatch)
{
    using Dense = typename TestFixture::Dense;
    auto sketch = TestFixture::Sketch::create(this->exec, 3, 5, 42);
...
TYPED_TEST(CountSketchCore, RapplyThrowsOnRowMismatch)
{
    using Dense = typename TestFixture::Dense;
    auto sketch = TestFixture::Sketch::create(this->exec, 3, 5, 42);
    auto b = Dense::create(this->exec, gko::dim<2>{4, 5});
