    ...
    ptr_param<AbstractMultiVector> output) const
{
    ...
    GKO_ASSERT_PRECISION(as_real(this->get_precision()), output);
    ...
    ptr_param<AbstractMultiVector> result) const
{
    ...
    GKO_ASSERT_PRECISION(as_complex(this->get_precision()), result);
    auto exec = this->get_executor();
...
{
    GKO_ASSERT_EQUAL_DIMENSIONS(this, result);
    GKO_ASSERT_PRECISION(as_real(this->get_precision()), result);
    auto exec = this->get_executor();
...
temporary_conversion<AbstractMultiVector> AbstractMultiVector::as_precision(
    precision p)
{
...
{
    if (this->get_precision() != precision_v<ValueType>) {
