...
LowerTrs<ValueType, IndexType>::LowerTrs(const LowerTrs& other)
    : EnablePolymorphicObject<LowerTrs, LinOp>(other.get_executor())
{
    *this = other;
}
...
{
    if (this != &other) {
        EnablePolymorphicObject<LowerTrs, LinOp>::operator=(other);
...
{
    if (this != &other) {
        EnablePolymorphicObject<LowerTrs, LinOp>::operator=(std::move(other));
