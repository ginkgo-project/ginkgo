                                     ...
                                     size_type next_nrows)
{
    ...
    if (scale_correction) {
        ...
        storage.template create_or_get_op<vec>(
            6, [&] { return vec::create(exec, dim<2>{current_nrows, nrhs}); },
            typeid(vec), dim<2>{current_nrows, nrhs}, nrhs);

        // Slot 7: dp = delta_pre / smoother scratch (current level)
        storage.template create_or_get_op<vec>(
            7, [&] { return vec::create(exec, dim<2>{current_nrows, nrhs}); },
            typeid(vec), dim<2>{current_nrows, nrhs}, nrhs);

        // Slot 8: alpha = Rayleigh numerator scalar (1 x nrhs)
        storage.template create_or_get_op<vec>(
            8, [&] { return vec::create(exec, dim<2>{1, nrhs}); }, typeid(vec),
            dim<2>{1, nrhs}, nrhs);

        // Slot 9: denom = Rayleigh denominator scalar (1 x nrhs)
        storage.template create_or_get_op<vec>(
            9, [&] { return vec::create(exec, dim<2>{1, nrhs}); }, typeid(vec),
            dim<2>{1, nrhs}, nrhs);
    }
}
