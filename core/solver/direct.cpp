    ...
    const Direct& other)
{
    if (this != &other) {
        ...
        lower_solver_ = clone(exec, other.lower_solver_);
        upper_solver_ = clone(exec, other.upper_solver_);
    }
    return *this;
}
...
template <typename ValueType, typename IndexType>
void Direct<ValueType, IndexType>::apply_impl(const AbstractMultiVector* b,
...
template <typename ValueType, typename IndexType>
void Direct<ValueType, IndexType>::apply_impl(const AbstractMultiVector* alpha,
                                              ...
                                              AbstractMultiVector* x) const
{
    ...
    auto dense_beta = as<matrix::MultiVector<ValueType>>(
        beta->as_precision(precision_v<ValueType>));
    auto intermediate = this->create_workspace_op_with_config_of(
        ws::intermediate, converted_b.get());
    lower_solver_->apply(converted_b.get(), intermediate);
