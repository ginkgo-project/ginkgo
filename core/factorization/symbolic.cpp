    ...
    std::unique_ptr<elimination_forest<IndexType>>& forest)
{
    ...
    const auto factor_nnz =
        static_cast<size_type>(row_ptrs.get_value(num_rows));
    ...
    std::unique_ptr<matrix::Csr<ValueType, IndexType>>& factors)
{
    ...
    const auto storage_size =
        static_cast<size_type>(storage_offsets.get_value(size[0]));
    ...
    const auto factor_nnz =
        static_cast<size_type>(factor_row_ptrs.get_value(size[0]));
