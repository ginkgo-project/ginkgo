...
{
    if (&other != this) {
        EnableClonableLinOp<SparsityCsr>::operator=(other);
        value_ = other.value_;
...
{
    if (&other != this) {
        EnableClonableLinOp<SparsityCsr>::operator=(std::move(other));
        value_ = other.value_;
...
SparsityCsr<ValueType, IndexType>::SparsityCsr(
    ...
    : EnableClonableLinOp<SparsityCsr>(exec, size),
...
SparsityCsr<ValueType, IndexType>::SparsityCsr(
    ...
    : EnableClonableLinOp<SparsityCsr>(exec, matrix->get_size())
{
