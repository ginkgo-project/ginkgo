...
Fft::Fft(std::shared_ptr<const Executor> exec, size_type size, bool inverse)
    : EnableLinOp<Fft>(exec, dim<2>{size}, precision::any),
      buffer_{exec},
      inverse_{inverse}
{}
...
Fft2::Fft2(std::shared_ptr<const Executor> exec, size_type size1,
    ...
    : EnableLinOp<Fft2>(exec, dim<2>{size1 * size2}, precision::any),
      buffer_{exec},
...
Fft3::Fft3(std::shared_ptr<const Executor> exec, size_type size1,
    ...
    : EnableLinOp<Fft3>(exec, dim<2>{size1 * size2 * size3}, precision::any),
      buffer_{exec},
