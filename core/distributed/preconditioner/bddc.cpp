...
// Realizes A_LL^{-1} (L = interior I + dual D) through a block factorization
// built on top of a single interior solver `inner_solver_` (= A_II^{-1}), so the
// interior solve is identical to the one used in the static condensation:
//   A_LL = | A_II  A_ID |   ->   A_LL^{-1} [f_I; f_D]:
//          | A_DI  A_DD |        g   = A_II^{-1} f_I
//                                u_D = S_DD^{-1} (f_D - A_DI g)
//                                u_I = g - A_II^{-1} (A_ID u_D)
// with the interior Schur complement S_DD = A_DD - A_DI A_II^{-1} A_ID solved by
// `schur_solver_`. When `inner_solver_` is exact this reproduces exact A_LL^{-1}.
template <typename ValueType, typename IndexType>
class BlockLLSolver
    : public gko::EnableLinOp<BlockLLSolver<ValueType, IndexType>>,
      public gko::EnableCreateMethod<BlockLLSolver<ValueType, IndexType>> {
public:
    using local_vec = gko::matrix::Dense<ValueType>;

    BlockLLSolver(std::shared_ptr<const gko::Executor> exec,
                  std::shared_ptr<const LinOp> inner_solver = nullptr,
                  std::shared_ptr<const LinOp> a_id = nullptr,
                  std::shared_ptr<const LinOp> a_di = nullptr,
                  std::shared_ptr<const LinOp> schur_solver = nullptr,
                  size_type n_inner = 0, size_type n_dual = 0)
        : gko::EnableLinOp<BlockLLSolver>(
              exec, gko::dim<2>{n_inner + n_dual, n_inner + n_dual}),
          inner_solver_{inner_solver},
          a_id_{a_id},
          a_di_{a_di},
          schur_solver_{schur_solver},
          n_inner_{n_inner},
          n_dual_{n_dual}
    {
        one_ = gko::initialize<local_vec>({1.0}, exec);
        neg_one_ = gko::initialize<local_vec>({-1.0}, exec);
        rhs_ = local_vec::create(exec, gko::dim<2>{n_inner + n_dual, 1});
        sol_ = local_vec::create(exec, gko::dim<2>{n_inner + n_dual, 1});
        g_ = local_vec::create(exec, gko::dim<2>{n_inner, 1});
        tmp_i_ = local_vec::create(exec, gko::dim<2>{n_inner, 1});
    }

protected:
    void apply_impl(const LinOp* b, LinOp* x) const override
    {
        // Copy into private buffers so that x may safely alias b.
        rhs_->copy_from(as<local_vec>(b));
        auto f_i = rhs_->create_submatrix(span{0, n_inner_}, span{0, 1});
        auto f_d = rhs_->create_submatrix(span{n_inner_, n_inner_ + n_dual_},
                                          span{0, 1});
        auto u_i = sol_->create_submatrix(span{0, n_inner_}, span{0, 1});
        auto u_d = sol_->create_submatrix(span{n_inner_, n_inner_ + n_dual_},
                                          span{0, 1});

        // g = A_II^{-1} f_I
        inner_solver_->apply(f_i.get(), g_.get());
        // f_d <- f_d - A_DI g  (reduced dual right-hand side)
        a_di_->apply(neg_one_.get(), g_.get(), one_.get(), f_d.get());
        // u_D = S_DD^{-1} (f_d)
        schur_solver_->apply(f_d.get(), u_d.get());
        // u_I = g - A_II^{-1} (A_ID u_D)
        a_id_->apply(u_d.get(), tmp_i_.get());
        inner_solver_->apply(tmp_i_.get(), u_i.get());
        u_i->scale(neg_one_.get());
        u_i->add_scaled(one_.get(), g_.get());

        as<local_vec>(x)->copy_from(sol_.get());
    }

    void apply_impl(const LinOp*, const LinOp* b, const LinOp*,
                    LinOp* x) const override
    {}

private:
    std::shared_ptr<const LinOp> inner_solver_;
    std::shared_ptr<const LinOp> a_id_;
    std::shared_ptr<const LinOp> a_di_;
    std::shared_ptr<const LinOp> schur_solver_;
    size_type n_inner_;
    size_type n_dual_;
    std::shared_ptr<local_vec> one_;
    std::shared_ptr<local_vec> neg_one_;
    std::shared_ptr<local_vec> rhs_;
    std::shared_ptr<local_vec> sol_;
    std::shared_ptr<local_vec> g_;
    std::shared_ptr<local_vec> tmp_i_;
};
    ...
    std::shared_ptr<const LinOp> system_matrix)
{
    ...
    if (active) {
        ...
        // Override local_solver_ (A_LL^{-1}) with a block factorization built on
        // top of inner_solver_ (A_II^{-1}), so the interior solve embedded in
        // every A_LL solve (Phi/Lambda setup, constraint Schur, substructure
        // correction) is identical to the one used in the static condensation.
        // This keeps the coarse operator consistent with the apply when the
        // interior solver is inexact. With an exact inner_solver_ this reproduces
        // exact A_LL^{-1}.
        {
            size_type n_dual_dofs = n_face_idxs + n_edge_idxs;
            if (A_II->get_size()[0] > 0 && n_dual_dofs > 0) {
                auto D_span = span{n_inner_idxs, n_inner_idxs + n_dual_dofs};
                auto A_ID = share(reordered_system_matrix->create_submatrix(
                    span{0, n_inner_idxs}, D_span));
                auto A_DI = share(reordered_system_matrix->create_submatrix(
                    D_span, span{0, n_inner_idxs}));
                auto A_DD = share(reordered_system_matrix->create_submatrix(
                    D_span, D_span));

                // A_II^{-1} A_ID, one inner solve per dual degree of freedom.
                auto A_ID_dense =
                    share(local_vec::create(exec, dim<2>{n_inner_idxs, 1}));
                A_ID->convert_to(A_ID_dense.get());
                auto Aii_inv_A_ID = share(
                    local_vec::create(exec, dim<2>{n_inner_idxs, n_dual_dofs}));
                Aii_inv_A_ID->fill(zero<ValueType>());
                for (size_type i = 0; i < n_dual_dofs; i++) {
                    auto rhs = share(A_ID_dense->create_submatrix(
                        span{0, n_inner_idxs}, span{i, i + 1}));
                    auto sol = share(Aii_inv_A_ID->create_submatrix(
                        span{0, n_inner_idxs}, span{i, i + 1}));
                    inner_solver_->apply(rhs, sol);
                }
                // S_DD = A_DD - A_DI (A_II^{-1} A_ID)
                auto S_DD = share(
                    local_vec::create(exec, dim<2>{n_dual_dofs, n_dual_dofs}));
                A_DD->convert_to(S_DD.get());
                A_DI->apply(neg_one_, Aii_inv_A_ID, one_, S_DD);

                std::shared_ptr<const LinOp> S_DD_solver =
