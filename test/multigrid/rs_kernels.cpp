...
TEST_F(Rs, GenerateMgLevelIsEquivalentToRef)
{
    gko::size_type c_size = 34;
    initialize_data(c_size);
    auto mg_level_factory = gko::multigrid::Rs<value_type, int>::build()
                                .with_coarse_rows(coarse_rows)
                                .with_skip_sorting(true)
                                .on(ref);
    auto d_mg_level_factory = gko::multigrid::Rs<value_type, int>::build()
...
TEST_F(Rs, GenerateMgLevelIsEquivalentToRefOnUnsortedMatrix)
{
    initialize_data(243);
