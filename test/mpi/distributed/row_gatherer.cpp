...
TYPED_TEST(RowGatherer, CanApplyAsync)
{
    ...
    auto x =
        Dense::create(this->mpi_exec, gko::dim<2>{this->rg->get_size()[0], 1});
    ...
    auto expected_vec = Dense::create(
        this->mpi_exec, gko::dim<2>{expected.get_size(), 1}, expected, 1);
...
TYPED_TEST(RowGatherer, CanApplyAsyncWithMultipleColumns)
{
    ...
    auto b = Vector::create(
        ...
        gko::initialize<Dense>({{offset, offset * offset},
                               ...
                               this->exec));
    auto x =
        Dense::create(this->mpi_exec, gko::dim<2>{this->rg->get_size()[0], 2});
    ...
    auto expected_vec = Dense::create(
        this->mpi_exec, gko::dim<2>{expected[rank].get_size() / 2, 2},
        expected[rank], 2);
    GKO_ASSERT_MTX_NEAR(x, expected_vec, 0.0);
}
