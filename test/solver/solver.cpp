...
// struct Chebyshev :
// SimpleSolverTest<gko::solver::Chebyshev<solver_value_type>> {
//     static double tolerance() { return 1e7 * r<value_type>::value; }
...
//     {
//         return
//         SimpleSolverTest<gko::solver::CbGmres<solver_value_type>>::build(
//                    exec, iteration_count, check_residual)
...
//     {
//         return
//         SimpleSolverTest<gko::solver::Gmres<solver_value_type>>::build(
//                    exec, iteration_count, check_residual)
//             .with_krylov_dim(dimension);
//     }
...
//                                status, bool all_stopped) const override
//     {
...
//                                const gko::size_type& num_bytes) const
//                                override
//     {
...
//                             gko::remove_complex<typename
//                             VecType::value_type>>( 0.0, 1.0),
//                         rand_engine)},
//                     ref),
//                 exec};
//     }
...
//     template <typename VecType = Vec, typename MtxOrSolver>
//     test_pair<VecType> gen_out_vec(const test_pair<MtxOrSolver>& op, int
//     nrhs,
//                                    int stride)
//     {
...
//                     ->generate(mtx.dev)});
//         }
//         static_assert(!(Config::requires_num_rhs() &&
//         Config::is_iterative()),
//                       "Inconsistent config");
...
//                 guarded_fn(
//                     test_pair<SolverType>{Config::build(ref, 4,
//                     check_residual)
//                                               .on(ref)
//                                               ->generate(mtx.ref),
//                                           Config::build(exec, 4,
...
//                                ? 2
//                                : 1);
//         // No 0x0 cleared/defaulted solvers, as they would be inconsistent
//         with
//         // `vec`
//         {
//             SCOPED_TRACE("Unpreconditioned solver with 0 iterations via
...
//                                  const test_pair<MtxType>& mtx, TestFunction
//                                  fn)
//     {
...
//                      Minres, LowerTrs, UpperTrs, LowerTrsUnitdiag,
//                      UpperTrsUnitdiag
// #ifdef GKO_COMPILING_CUDA
//                      ,
...
//                         const gko::matrix::Identity<typename
//                         Mtx::value_type>*>(
//                 precond));
//         }
//         GKO_ASSERT_MTX_NEAR(gko::as<Mtx>(solver->get_system_matrix()),
//         mtx.ref,
//                             0.0);
//     });
// }
...
//                 cross-executor auto precond =
//                 Config::get_preconditioner(solver2); if (dynamic_cast<const
//                 Precond*>(precond)) {
//                     GKO_ASSERT_MTX_NEAR(
...
//                                     gko::as<Mtx>(mtx), 0.0);
//                 // TODO no easy way to compare stopping criteria
//                 cross-executor auto new_precond =
//                 Config::get_preconditioner(solver2); if (dynamic_cast<const
//                 Precond*>(new_precond)) {
