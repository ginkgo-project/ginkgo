...
#if GKO_HAVE_CUDSS && defined(GKO_COMPILING_CUDA)


class DirectVendor : public CommonTestFixture {
protected:
    ...
    std::default_random_engine rand_engine;
    std::unique_ptr<typename solver_type::Factory> ref_factory;
    std::unique_ptr<typename solver_type::Factory> vendor_factory;
    std::shared_ptr<matrix_type> mtx;
    std::shared_ptr<matrix_type> dmtx;
    std::shared_ptr<vector_type> alpha;
    std::shared_ptr<vector_type> beta;
...
TEST_F(DirectVendor, VendorApplyToSingleRhsMatchesRef)
{
    this->initialize_data(gko::matrices::location_ani4_amd_mtx, 1);
    auto ref_solver = this->ref_factory->generate(this->mtx);
    auto vendor_solver = this->vendor_factory->generate(this->dmtx);

    ref_solver->apply(this->input, this->output);
    vendor_solver->apply(this->dinput, this->doutput);

    GKO_ASSERT_MTX_NEAR(this->output, this->doutput, 100 * r<double>::value);
}


TEST_F(DirectVendor, VendorApplyToMultipleRhsMatchesRef)
{
    this->initialize_data(gko::matrices::location_ani4_amd_mtx, 6);
    auto ref_solver = this->ref_factory->generate(this->mtx);
    auto vendor_solver = this->vendor_factory->generate(this->dmtx);
...
TEST_F(DirectVendor, VendorAdvancedApplyMatchesRef)
{
    this->initialize_data(gko::matrices::location_ani4_amd_mtx, 1);
    auto ref_solver = this->ref_factory->generate(this->mtx);
    auto vendor_solver = this->vendor_factory->generate(this->dmtx);
    ...
    vendor_solver->apply(this->dalpha, this->dinput, this->dbeta,
                         this->doutput);
