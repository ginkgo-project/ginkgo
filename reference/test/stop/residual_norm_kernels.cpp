...
TYPED_TEST(ResidualNorm, CheckIfResZeroConverges)
{
    ...
    for (auto baseline :
        ...
        gko::array<bool> stop_indicators(this->exec_->get_master(), 2);
...
TYPED_TEST(ResidualNorm, WaitsTillResidualGoal)
{
        ...
        this->abs_factory_->generate(nullptr, rhs, nullptr, initial_res.get());
    {
        ...
        ASSERT_EQ(stop_indicators.get_const_data()[0], false);
        ...
        ASSERT_EQ(stop_indicators.get_const_data()[0], true);
    }
    {
...
TYPED_TEST(ResidualNorm, RelativeSelfCalculatesThrowWithoutRhs)
{
    ...
    ASSERT_THROW(
        rel_criterion->update().solution(solution).check(
            RelativeStoppingId, true, &stop_status, stop_indicators.get_data()),
        gko::NotSupported);
}
