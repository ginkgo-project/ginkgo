...
opts = { \
         "marklist" : ['o', 'x', 'v', '+', '^', '+','x','v'],
         "colorlist" : ['mediumblue', 'red', 'forestgreen', 'c', 'm', 'k','orange','red'],
         "linetype" : ['-', '--', '-.', ':', '--','--','-'],
         "linewidth" : 0.75,
...
df3 = df3[~df3["batch_size"].isin(batch_list)]
df3 = df3[~df3["nrows"].isin(row_list)]
...
df.plot(kind='bar', rot=45, xlabel='Num batch entries', ylabel='Speedup',colormap='Dark2')
# # add some labels
