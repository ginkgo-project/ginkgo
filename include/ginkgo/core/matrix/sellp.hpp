...
#if GINKGO_ENABLE_HALF || GINKGO_ENABLE_BFLOAT16
    friend class Sellp<previous_precision_move<ValueType, 2>, IndexType>;
    using ConvertibleTo<
        Sellp<next_precision_move<ValueType, 2>, IndexType>>::convert_to;
    using ConvertibleTo<
        Sellp<next_precision_move<ValueType, 2>, IndexType>>::move_to;

    void convert_to(Sellp<next_precision_move<ValueType, 2>, IndexType>* result)
        const override;
