...
class Fbcsr
    : public EnableLinOp<Fbcsr<ValueType, IndexType>>,
      ...
      public ConvertibleTo<Fbcsr<next_precision_move<ValueType, 2>, IndexType>>,
#endif
#if GINKGO_ENABLE_HALF && GINKGO_ENABLE_BFLOAT16
      public ConvertibleTo<Fbcsr<next_precision_move<ValueType, 3>, IndexType>>,
#endif
...
#if GINKGO_ENABLE_HALF || GINKGO_ENABLE_BFLOAT16
    friend class Fbcsr<previous_precision_move<ValueType, 2>, IndexType>;
    using ConvertibleTo<
        Fbcsr<next_precision_move<ValueType, 2>, IndexType>>::convert_to;
    using ConvertibleTo<
        Fbcsr<next_precision_move<ValueType, 2>, IndexType>>::move_to;

    void convert_to(Fbcsr<next_precision_move<ValueType, 2>, IndexType>* result)
        const override;

    void move_to(
        Fbcsr<next_precision_move<ValueType, 2>, IndexType>* result) override;
#endif

#if GINKGO_ENABLE_HALF && GINKGO_ENABLE_BFLOAT16
    friend class Fbcsr<previous_precision_move<ValueType, 3>, IndexType>;
    using ConvertibleTo<
        Fbcsr<next_precision_move<ValueType, 3>, IndexType>>::convert_to;
    using ConvertibleTo<
        Fbcsr<next_precision_move<ValueType, 3>, IndexType>>::move_to;

    void convert_to(Fbcsr<next_precision_move<ValueType, 3>, IndexType>* result)
        const override;

    void move_to(
        Fbcsr<next_precision_move<ValueType, 3>, IndexType>* result) override;
#endif
