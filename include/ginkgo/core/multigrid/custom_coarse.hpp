...
class CustomCoarse : public EnableLinOp<CustomCoarse<ValueType>>,
...
protected:
    ...
    explicit CustomCoarse(std::shared_ptr<const Executor> exec)
        : EnableLinOp<CustomCoarse>(std::move(exec))
    {}

    explicit CustomCoarse(const Factory* factory,
                          std::shared_ptr<const LinOp> system_matrix)
        : EnableLinOp<CustomCoarse>(factory->get_executor(),
