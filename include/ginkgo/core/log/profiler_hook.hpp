...
public:
    ...
    GKO_EXPORT void on_allocation_started(const gko::Executor* exec,
                                          const gko::size_type&) const override;

    GKO_EXPORT void on_allocation_completed(const gko::Executor* exec,
                                            const gko::size_type&,
                                            const gko::uintptr&) const override;

    GKO_EXPORT void on_free_started(const gko::Executor* exec,
                                    const gko::uintptr&) const override;

    GKO_EXPORT void on_free_completed(const gko::Executor* exec,
                                      const gko::uintptr&) const override;

    GKO_EXPORT void on_copy_started(const gko::Executor* from,
                                    const gko::Executor* to,
                                    const gko::uintptr&, const gko::uintptr&,
                                    const gko::size_type&) const override;

    GKO_EXPORT void on_copy_completed(const gko::Executor* from,
                                      const gko::Executor* to,
                                      const gko::uintptr&, const gko::uintptr&,
                                      const gko::size_type&) const override;
    ...
    GKO_EXPORT bool needs_propagation() const override;
     ...
     * @param name  its name
     */
    GKO_EXPORT void set_object_name(ptr_param<const PolymorphicObject> obj,
                                    std::string name);
     ...
     */
    GKO_EXPORT void set_synchronization(bool synchronize);
     ...
     */
    GKO_EXPORT profiling_scope_guard user_range(const char* name) const;
     ...
     */
    GKO_EXPORT static std::shared_ptr<ProfilerHook> create_tau(
        bool initialize = true);

    /**
     * Creates a logger annotating Ginkgo events with VTune ITT ranges.
     */
    GKO_EXPORT static std::shared_ptr<ProfilerHook> create_vtune();
     ...
     */
    GKO_EXPORT static std::shared_ptr<ProfilerHook> create_nvtx(
        uint32 color_argb = color_yellow_argb);

    /**
     * Creates a logger annotating Ginkgo events with ROCTX ranges for HIP.
     */
    GKO_EXPORT static std::shared_ptr<ProfilerHook> create_roctx();
     ...
     */
    GKO_EXPORT static std::shared_ptr<ProfilerHook> create_for_executor(
     ...
     */
    GKO_EXPORT static std::shared_ptr<ProfilerHook> create_summary(
     ...
     *       `set_synchronization(true)`.
     */
    GKO_EXPORT static std::shared_ptr<ProfilerHook> create_nested_summary(
        std::shared_ptr<Timer> timer = std::make_shared<CpuTimer>(),
     ...
     */
    GKO_EXPORT static std::shared_ptr<ProfilerHook> create_custom(
        hook_function begin, hook_function end);

private:
    ProfilerHook(hook_function begin, hook_function end);

    GKO_EXPORT void maybe_synchronize(const Executor* exec) const;
...
public:
    ...
    GKO_EXPORT profiling_scope_guard();
     ...
     */
    GKO_EXPORT profiling_scope_guard(const char* name,
    ...
    GKO_EXPORT profiling_scope_guard(const profiling_scope_guard&) = delete;

    GKO_EXPORT profiling_scope_guard(profiling_scope_guard&& other) = delete;

    GKO_EXPORT profiling_scope_guard& operator=(const profiling_scope_guard&) =
        delete;

    GKO_EXPORT profiling_scope_guard& operator=(profiling_scope_guard&&) =
        delete;
