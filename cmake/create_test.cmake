...
function(ginkgo_build_test_name test_name target_name)
    cmake_parse_arguments(
        PARSE_ARGV 2 build_test_name "" "${gko_test_single_args}" ""
    )
...
# Set up shared target properties and handle
# ADDITIONAL_LIBRARIES/ADDITIONAL_INCLUDES `MPI_SIZE size` causes the tests to
# be run with `size` MPI processes.
function(ginkgo_set_test_target_properties test_target_name test_library_suffix)
    cmake_parse_arguments(
        PARSE_ARGV 1 set_properties "${gko_test_option_args}"
        "${gko_test_single_args}" "${gko_test_multi_args}"
    )
    if(GINKGO_FAST_TESTS)
        target_compile_definitions(
            ${test_target_name} PRIVATE GINKGO_FAST_TESTS
        )
    endif()
    if(GINKGO_TEST_NONDEFAULT_STREAM)
        target_compile_definitions(
            ${test_target_name} PRIVATE GKO_TEST_NONDEFAULT_STREAM
        )
    endif()
    if(GINKGO_COMPILING_DPCPP_TEST AND GINKGO_DPCPP_SINGLE_MODE)
        target_compile_definitions(
            ${test_target_name} PRIVATE GINKGO_DPCPP_SINGLE_MODE=1
        )
    endif()
    if(GINKGO_CHECK_CIRCULAR_DEPS)
        target_link_libraries(
            ${test_target_name} PRIVATE "${GINKGO_CIRCULAR_DEPS_FLAGS}"
        )
    endif()
    ...
    # we set these properties regardless of the enabled backends, because
    ...
    target_include_directories(
        ${test_target_name} PRIVATE ${Ginkgo_BINARY_DIR}
                                    ${set_properties_ADDITIONAL_INCLUDES}
    )
    target_link_libraries(
        ${test_target_name} PRIVATE ginkgo GTest::GTest
                                    ${set_properties_ADDITIONAL_LIBRARIES}
    )
endfunction()

function(ginkgo_add_resource_requirement test_name)
    cmake_parse_arguments(
        PARSE_ARGV 1 add_rr "${gko_test_option_args}" "${gko_test_single_args}"
        ""
    )
    ...
    set_property(
        TEST ${test_name} PROPERTY RESOURCE_GROUPS
                                   "${add_rr_MPI_SIZE},${single_resource}"
    )
endfunction()

# Adds a test to the list executed by ctest and sets its output binary name
# Possible additional arguments: - `MPI_SIZE size` causes the tests to be run
# with `size` MPI processes. - `RESOURCE_LOCAL_CORES` the number of threads used
# by a test, default is $GINKGO_CI_TEST_OMP_PARALLELISM - `RESOURCE_TYPE` the
# resource type, can be cpu, cudagpu, hipgpu, sycl - `DISABLE_EXECUTORS exec1
# exec2` disables the test for certain backends (if built for multiple) -
...
function(ginkgo_add_test test_name test_target_name)
    cmake_parse_arguments(
        PARSE_ARGV 2 add_test "" "${gko_test_single_args}"
        "${gko_test_multi_args}"
    )
    file(RELATIVE_PATH REL_BINARY_DIR ${PROJECT_BINARY_DIR}
         ${CMAKE_CURRENT_BINARY_DIR}
    ...
    set_target_properties(
        ${test_target_name} PROPERTIES OUTPUT_NAME ${test_binary_name}
    )
    ...
    ginkgo_add_resource_requirement(
        ${REL_BINARY_DIR}/${test_binary_name} ${ARGN}
    )
...
function(ginkgo_create_dpcpp_test test_name)
    ...
    target_link_options(
        ${test_target_name} PRIVATE -fsycl-device-code-split=per_kernel
    )
    ...
    if(MKL_ENV)
        set_tests_properties(
            ${test_target_name} PROPERTIES ENVIRONMENT "${MKL_ENV}"
        )
    endif()
...
function(ginkgo_create_cuda_test test_name)
    ...
    ginkgo_create_cuda_test_internal(
        ${test_name} ${test_name}.cu ${test_target_name} ${ARGN}
    )
...
function(ginkgo_create_cuda_test_internal test_name filename test_target_name)
    ...
    target_compile_definitions(
        ${test_target_name} PRIVATE GKO_COMPILING_CUDA
                                    GKO_DEVICE_NAMESPACE=cuda
    )
    if(GINKGO_CUDA_CUSTOM_THRUST_NAMESPACE)
        target_compile_definitions(
            ${test_target_name} PRIVATE THRUST_CUB_WRAPPED_NAMESPACE=gko
        )
    endif()
    if(MSVC)
    ...
    ginkgo_add_test(
        ${test_name} ${test_target_name} ${ARGN} RESOURCE_TYPE cudagpu
    )
...
function(ginkgo_create_hip_test test_name)
    ...
    ginkgo_create_hip_test_internal(
        ${test_name} ${test_name}.hip.cpp ${test_target_name} ${ARGN}
    )
...
function(ginkgo_create_hip_test_internal test_name filename test_target_name)
    ...
    add_executable(${test_target_name} ${filename})
    target_compile_definitions(
        ${test_target_name} PRIVATE GKO_COMPILING_HIP GKO_DEVICE_NAMESPACE=hip
    )
    ...
    ginkgo_add_test(
        ${test_name} ${test_target_name} ${ARGN} RESOURCE_TYPE hipgpu
    )
...
function(ginkgo_create_omp_test test_name)
    ...
    ginkgo_create_omp_test_internal(
        ${test_name} ${test_name}.cpp ${test_target_name} "" ${ARGN}
    )
endfunction()
...
function(ginkgo_create_omp_test_internal test_name filename test_target_name)
    ...
    add_executable(${test_target_name} ${test_name}.cpp)
    target_compile_definitions(
        ${test_target_name} PRIVATE GKO_COMPILING_OMP GKO_DEVICE_NAMESPACE=omp
    )
...
# Common test compiled with the host compiler, one target for each enabled
# backend
function(ginkgo_create_common_test test_name)
    ...
    if(GINKGO_BUILD_CUDA)
        ginkgo_create_common_test_internal(
            ${test_name} CudaExecutor cuda ${ARGN}
        )
    endif()
    if(GINKGO_BUILD_SYCL)
        ginkgo_create_common_test_internal(
            ${test_name} DpcppExecutor dpcpp ${ARGN}
        )
    endif()
...
function(ginkgo_create_common_test_internal test_name exec_type exec)
    cmake_parse_arguments(
        PARSE_ARGV 3 common_test "" "${gko_test_single_args}"
        "${gko_test_multi_args}"
    )
    ...
    target_compile_definitions(
        ${test_target_name}
        PRIVATE EXEC_TYPE=${exec_type} GKO_DEVICE_NAMESPACE=${exec}
                GKO_COMPILING_${exec_upper}
    )
    target_link_libraries(
        ${test_target_name} PRIVATE ${common_test_ADDITIONAL_LIBRARIES}
    )
    # use float for DPC++ if necessary
    if((exec STREQUAL "dpcpp") AND GINKGO_DPCPP_SINGLE_MODE)
        target_compile_definitions(
            ${test_target_name} PRIVATE GINKGO_COMMON_SINGLE_MODE=1
        )
        target_compile_definitions(
            ${test_target_name} PRIVATE GINKGO_DPCPP_SINGLE_MODE=1
        )
    ...
    ginkgo_add_test(
        ${test_name}_${exec} ${test_target_name} ${ARGN} RESOURCE_TYPE
        ${test_resource_type}
    )
endfunction(ginkgo_create_common_test_internal)

# Common test compiled with the device compiler, one target for each enabled
# backend
function(ginkgo_create_common_device_test test_name)
    cmake_parse_arguments(
        PARSE_ARGV 1 common_device_test "" "${gko_test_single_args}"
        "${gko_test_multi_args}"
    )
    ginkgo_build_test_name(${test_name} test_target_name ${ARGN})
    if(GINKGO_BUILD_SYCL)
        ...
        target_compile_options(
            ${test_target_name}_dpcpp PRIVATE ${GINKGO_DPCPP_FLAGS}
        )
        # We need to use a new file to avoid sycl setting in other backends
        # because add_sycl_to_target will change the source property.
        configure_file(${test_name}.cpp ${test_name}.dp.cpp COPYONLY)
        gko_add_sycl_to_target(
            TARGET ${test_target_name}_dpcpp SOURCES ${test_name}.dp.cpp
        )
        target_link_options(
            ${test_target_name}_dpcpp PRIVATE -fsycl-device-lib=all
    ...
    if(GINKGO_BUILD_CUDA)
        # need to make a separate file for this, since we can't set conflicting
        # properties on the same file
        configure_file(${test_name}.cpp ${test_name}.cu COPYONLY)
        ginkgo_create_cuda_test_internal(
            ${test_name}_cuda ${CMAKE_CURRENT_BINARY_DIR}/${test_name}.cu
            ${test_target_name}_cuda ${ARGN}
        )
        target_compile_definitions(
            ${test_target_name}_cuda PRIVATE EXEC_TYPE=CudaExecutor
                                             GKO_DEVICE_NAMESPACE=cuda
        )
    endif()
    if(GINKGO_BUILD_HIP)
        ...
        ginkgo_create_hip_test_internal(
            ${test_name}_hip ${CMAKE_CURRENT_BINARY_DIR}/${test_name}.hip.cpp
            ${test_target_name}_hip ${ARGN}
        )
        target_compile_definitions(
            ${test_target_name}_hip PRIVATE EXEC_TYPE=HipExecutor
                                            GKO_DEVICE_NAMESPACE=hip
        )
    endif()
endfunction(ginkgo_create_common_device_test)

# Common test compiled with the host compiler for all enabled backends and
# Reference
function(ginkgo_create_common_and_reference_test test_name)
    ginkgo_create_common_test(${test_name} ${ARGN})
    ginkgo_create_common_test_internal(
        ${test_name} ReferenceExecutor reference REFERENCE ${ARGN}
    )
