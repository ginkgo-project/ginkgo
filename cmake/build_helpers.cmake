...
function(ginkgo_default_includes name)
    ...
    if(GINKGO_HAVE_HWLOC)
        target_include_directories(
            "${name}"
            PUBLIC $<BUILD_INTERFACE:${HWLOC_INCLUDE_DIRS}>
        )
    endif()
endfunction()
...
function(ginkgo_compile_features name)
    ...
    if(GINKGO_WITH_CLANG_TIDY AND GINKGO_CLANG_TIDY_PATH)
        set_property(
            TARGET "${name}"
            PROPERTY CXX_CLANG_TIDY "${GINKGO_CLANG_TIDY_PATH};-checks=*"
        )
    endif()
    if(GINKGO_WITH_IWYU AND GINKGO_IWYU_PATH)
        set_property(
            TARGET "${name}"
            PROPERTY CXX_INCLUDE_WHAT_YOU_USE ${GINKGO_IWYU_PATH}
        )
    endif()
    ...
    set_property(
        TARGET "${name}"
        PROPERTY RUNTIME_OUTPUT_DIRECTORY "${GINKGO_LIBRARY_PATH}"
    )
    set_property(
        TARGET "${name}"
        PROPERTY ARCHIVE_OUTPUT_DIRECTORY "${GINKGO_LIBRARY_PATH}"
    )
    set_property(
        TARGET "${name}"
        PROPERTY LIBRARY_OUTPUT_DIRECTORY "${GINKGO_LIBRARY_PATH}"
    )
...
function(ginkgo_extract_clang_version CLANG_COMPILER GINKGO_CLANG_VERSION)
    ...
    file(
        WRITE
        "${CMAKE_CURRENT_BINARY_DIR}/extract_clang_ver.cpp"
        ${CLANG_VERSION_PROG}
    )
    execute_process(
        COMMAND
...
function(
    ...
    file(
        WRITE
        "${CMAKE_CURRENT_BINARY_DIR}/extract_dpcpp_ver.cpp"
        ${DPCPP_VERSION_PROG}
    )
    # we always add -fsycl
    execute_process(
