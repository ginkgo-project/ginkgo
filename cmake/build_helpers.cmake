...
function(ginkgo_default_includes name)
    ...
    target_include_directories(
        "${name}"
        PUBLIC $<BUILD_INTERFACE:${Ginkgo_BINARY_DIR}/include>
    ...
    if(GINKGO_HAVE_HWLOC)
        target_include_directories(
            "${name}" PUBLIC $<BUILD_INTERFACE:${HWLOC_INCLUDE_DIRS}>
        )
    endif()
endfunction()
...
function(ginkgo_compile_features name)
    ...
    if(GINKGO_WITH_CLANG_TIDY AND GINKGO_CLANG_TIDY_PATH)
        set_property(
            TARGET "${name}" PROPERTY CXX_CLANG_TIDY
                                      "${GINKGO_CLANG_TIDY_PATH};-checks=*"
        )
    endif()
    if(GINKGO_WITH_IWYU AND GINKGO_IWYU_PATH)
        set_property(
            TARGET "${name}" PROPERTY CXX_INCLUDE_WHAT_YOU_USE
                                      ${GINKGO_IWYU_PATH}
        )
    endif()
    # Set an appropriate SONAME
    set_property(TARGET "${name}" PROPERTY SOVERSION "${Ginkgo_VERSION}")
    # Put all shared libraries and corresponding imported libraries into the
    # specified path
    set_property(
        TARGET "${name}" PROPERTY RUNTIME_OUTPUT_DIRECTORY
                                  "${GINKGO_LIBRARY_PATH}"
    )
    set_property(
        TARGET "${name}" PROPERTY ARCHIVE_OUTPUT_DIRECTORY
                                  "${GINKGO_LIBRARY_PATH}"
    )
    set_property(
        TARGET "${name}" PROPERTY LIBRARY_OUTPUT_DIRECTORY
                                  "${GINKGO_LIBRARY_PATH}"
    )
...
function(ginkgo_check_headers target defines)
    # build object library used to "compile" the headers add a proxy source file
    # for each header in the target source list
    file(
        GLOB_RECURSE CUDA_HEADERS
        RELATIVE "${CMAKE_CURRENT_SOURCE_DIR}"
        CONFIGURE_DEPENDS "*.cuh"
    )
    file(
        GLOB_RECURSE HIP_HEADERS
        RELATIVE "${CMAKE_CURRENT_SOURCE_DIR}"
        CONFIGURE_DEPENDS "*.hip.hpp"
    )
    file(
    ...
    if(SOURCES)
        ...
        target_include_directories(
            ${target}_headers PRIVATE "${CMAKE_CURRENT_SOURCE_DIR}"
        )
        if(defines)
    ...
    if(HIP_SOURCES)
        ...
        add_library(
            ${target}_headers_hip ${HIP_SOURCES}
        ) # the compiler options get set by linking to ginkgo_hip
        target_link_libraries(
            ${target}_headers_hip PRIVATE ${target} roc::hipblas roc::hipsparse
                                          hip::hiprand roc::rocrand
        )
        target_include_directories(
            ${target}_headers_hip PRIVATE "${CMAKE_CURRENT_SOURCE_DIR}"
        )
    endif()
endfunction()
...
function(ginkgo_extract_clang_version CLANG_COMPILER GINKGO_CLANG_VERSION)
    ...
    execute_process(
        COMMAND
            ${CLANG_COMPILER} ${CMAKE_CURRENT_BINARY_DIR}/extract_clang_ver.cpp
...
function(ginkgo_extract_dpcpp_version DPCPP_COMPILER GINKGO_DPCPP_VERSION
    ...
    set(DPCPP_VERSION_PROG
        "#include <sycl/sycl.hpp>\n#include <iostream>\n"
        "int main() {std::cout << ${MACRO_VAR} << '\\n'\;" "return 0\;}"
    )
    file(WRITE "${CMAKE_CURRENT_BINARY_DIR}/extract_dpcpp_ver.cpp"
         ${DPCPP_VERSION_PROG}
    )
    # we always add -fsycl
    execute_process(
        COMMAND
            ${DPCPP_COMPILER} -fsycl
            ${CMAKE_CURRENT_BINARY_DIR}/extract_dpcpp_ver.cpp -o
            ${CMAKE_CURRENT_BINARY_DIR}/extract_dpcpp_ver
        ERROR_VARIABLE DPCPP_EXTRACT_VER_ERROR
    )
    execute_process(
        COMMAND ${CMAKE_CURRENT_BINARY_DIR}/extract_dpcpp_ver
