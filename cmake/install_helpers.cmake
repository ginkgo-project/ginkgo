...
function(ginkgo_add_install_rpath name)
    ...
    if(GINKGO_INSTALL_RPATH)
        set_property(
            TARGET "${name}" PROPERTY INSTALL_RPATH "${ORIGIN_OR_LOADER_PATH}"
                                      "${RPATH_DEPENDENCIES}"
        )
    endif()
endfunction()
...
#
# @param name  the name of the Ginkgo library target @param ARGN  this should
# contain any external dependency's library PATH
function(ginkgo_install_library name)
    ...
    install(
        ...
        RUNTIME DESTINATION "${CMAKE_INSTALL_BINDIR}" COMPONENT Ginkgo_Runtime
...
function(ginkgo_install)
    # static linking with pkg-config is not possible with HIP, since some linker
    # information cannot be expressed in pkg-config files
    if(BUILD_SHARED_LIBS OR NOT GINKGO_BUILD_HIP)
        # generate pkg-config file, a three-step process is necessary to include
        # the correct install prefix Step 1: substitute project variables in the
        # generation script
        configure_file(
            "${Ginkgo_SOURCE_DIR}/cmake/generate_pkg.cmake.in"
            "${Ginkgo_BINARY_DIR}/cmake/generate_pkg.cmake" @ONLY
        )
        # Step 2: substitute generator expressions
        file(
            GENERATE
            OUTPUT ${Ginkgo_BINARY_DIR}/cmake/generate_pkg_$<CONFIG>.cmake
            INPUT ${Ginkgo_BINARY_DIR}/cmake/generate_pkg.cmake
        )
        # Step 3: at install time, call the generation script which has all
        # variables except the install prefix already replaced. Use the install
        # prefix that is specified at install time
    ...
    if(GINKGO_HAVE_HWLOC AND NOT HWLOC_FOUND)
        ...
        install(
            DIRECTORY "${HWLOC_INCLUDE_DIRS}/hwloc"
            DESTINATION "${CMAKE_INSTALL_INCLUDEDIR}"
            COMPONENT Ginkgo_Development
        )
        install(
            FILES "${HWLOC_INCLUDE_DIRS}/hwloc.h"
            DESTINATION "${CMAKE_INSTALL_INCLUDEDIR}"
            COMPONENT Ginkgo_Development
        )
    endif()
    ...
    install(
        DIRECTORY "${Ginkgo_SOURCE_DIR}/cmake/Modules/"
    ...
    configure_package_config_file(
        ...
        PATH_VARS CMAKE_INSTALL_INCLUDEDIR CMAKE_INSTALL_LIBDIR
                  CMAKE_INSTALL_PREFIX GINKGO_INSTALL_MODULE_DIR
    )
    install(
        FILES "${Ginkgo_BINARY_DIR}/cmake/GinkgoConfig.cmake"
              "${Ginkgo_BINARY_DIR}/cmake/GinkgoConfigVersion.cmake"
    ...
    if(CMAKE_SYSTEM_NAME STREQUAL "Linux" AND BUILD_SHARED_LIBS)
        install(
            FILES "${Ginkgo_SOURCE_DIR}/dev_tools/scripts/gdb-ginkgo.py"
            DESTINATION "${CMAKE_INSTALL_LIBDIR}"
