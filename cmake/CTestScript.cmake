# .rst: Ginkgo CTestScript
# -------
#
# Runs our tests through CTest, with support for Coverage or memory checking.
#
# This script provides a full CTest run with result submission to Ginkgo's CDash
...
# without valgrind, requires the valgrind tool.
#
# Note that only one of these can be ran at once, as the build types conflict.
# Ginkgo is always configured with CUDA, HIP, OpenMP and Reference support,
# except for ThreadSanitizer, AddressSanitizer, LeakSanitizer,
...
# ^^^^^^^^^^^^^^^^^^
#
# To run the script, launch the command `ctest -S cmake/CTestScript.cmake` from
# the Ginkgo's source directory. The default settings are for the CI system's
# DEBUG tests run. To configure the script use standard CMake `-D` parameters.
# For example, the following command runs coverage.
#
...
#
# ``CTEST_SOURCE_DIRECTORY`` Where the sources are located. By default, the
# current directory.
#
# ``CTEST_BINARY_DIRECTORY`` In which directory should the sources be builts.
# Default, `./build`.
#
# ``CTEST_SITE`` A string to describe the machine this is ran on. Default
# FineCI.
#
# ``CTEST_CMAKE_GENERATOR`` Which generator should be used for the build.
# Default `Ninja`, except for COVERAGE builds where `Unix Makefiles` is used.
#
# ``CTEST_BUILD_CONFIGURATION`` Which configuration should Ginkgo be built with.
# Default `DEBUG`. The supported values are: COVERAGE, TSAN, UBSAN, DEBUG, and
# RELEASE.
#
# ``CTEST_TEST_MODEL`` Which CTest test model should be used. Default
# `Continuous`. The supported values are the same as CTest's, namely:
# Experimental, Nightly, Continuous.
#
# ``CTEST_BUILD_NAME`` The name of the build being ran. Default:
# `CTEST_BUILD_CONFIGURATION`
#
# ``CTEST_MEMORYCHECK_TYPE`` Whether memorycheck should be ran. Default: `NONE`.
# Supported values are: Valgrind, AddressSanitizer, LeakSanitizer,
# ThreadSanitizer, UndefinedBehaviorSanitizer and NONE.
#
...
# Supported: Valgrind, ThreadSanitizer, AddressSanitizer, LeakSanitizer and
...
if(CTEST_MEMORYCHECK_TYPE STREQUAL "Valgrind")
    ...
    set(CTEST_MEMORYCHECK_SUPPRESSIONS_FILE
        "${CTEST_SOURCE_DIRECTORY}/dev_tools/valgrind/suppressions.supp"
    )
endif()
...
if(NOT CTEST_MEMORYCHECK_TYPE STREQUAL "Valgrind"
   AND NOT CTEST_MEMORYCHECK_TYPE STREQUAL "CudaMemcheck"
)
    set(CTEST_MEMORYCHECK_SANITIZER_OPTIONS
        "${CTEST_MEMORYCHECK_SANITIZER_OPTIONS}:allocator_may_return_null=1:verbosity=1"
    )
endif()
...
elseif((NOT CTEST_MEMORYCHECK_TYPE STREQUAL "NONE"
        AND NOT CTEST_MEMORYCHECK_TYPE STREQUAL "Valgrind")
       OR CTEST_BUILD_CONFIGURATION STREQUAL "COVERAGE"
)
    set(GINKGO_CONFIGURE_OPTIONS
    ...
    set(GINKGO_CONFIGURE_OPTIONS
        "-DGINKGO_DEVEL_TOOLS=OFF;-DGINKGO_BUILD_REFERENCE=ON;-DGINKGO_BUILD_OMP=ON;-DGINKGO_BUILD_CUDA=ON;-DGINKGO_BUILD_HIP=ON;-DGINKGO_BUILD_SYCL=OFF;-DCMAKE_BUILD_TYPE=${CTEST_BUILD_CONFIGURATION}"
    )
endif()
...
if(CTEST_MEMORYCHECK_TYPE STREQUAL "UndefinedBehaviorSanitizer")
    set(GINKGO_CONFIGURE_OPTIONS
        "${GINKGO_CONFIGURE_OPTIONS};-DCMAKE_SHARED_LINKER_FLAGS=-fuse-ld=gold;-DCMAKE_EXE_LINKER_FLAGS=-fuse-ld=gold"
    )
endif()

ctest_configure(
    BUILD "${CTEST_BINARY_DIRECTORY}"
    OPTIONS "${GINKGO_CONFIGURE_OPTIONS}"
    APPEND
)
...
if(DEFINED GINKGO_SONARQUBE_TEST)
    set(CTEST_BUILD_COMMAND
        "build-wrapper-linux-x86-64 --out-dir bw-output make -j${PROC_COUNT}"
    )
endif()
...
if(NOT CTEST_MEMORYCHECK_TYPE STREQUAL "NONE")
    if(CTEST_MEMORYCHECK_TYPE STREQUAL "CudaMemcheck")
        ctest_memcheck(
            BUILD "${CTEST_BINARY_DIRECTORY}" INCLUDE "^(cuda|hip).*" APPEND
        )
    else()
