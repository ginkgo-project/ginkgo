function(add_instantiation_files source_dir source_file output_files_var)
    ...
    foreach(range RANGE 0 ${range_count_minus_one})
        ...
        string(REGEX REPLACE "(\.hip\.cpp|\.dp\.cpp|\.cpp|\.cu)$"
                             ".${range}\\1" target_file "${source_file}"
        )
        ...
        # concatenate header, content and footer and turn semicolons into
        # newlines
        string(REPLACE ";" "\n" content "${header};${content};${footer}")
        # and escaped semicolons into regular semicolons again
        string(REPLACE "<semicolon>" ";" content "${content}")
        # create a .tmp file, but only copy it over if source file changed this
        # way, we don't rebuild unnecessarily
        file(WRITE "${target_path}.tmp" "${content}")
        add_custom_command(
            OUTPUT "${target_path}"
            COMMAND ${CMAKE_COMMAND} -E copy "${target_path}.tmp"
                    "${target_path}"
