...
# This can't be replaced by `file(GENERATE ... TARGET ...)`, since the generator
# expressions might contain `COMPILE_LANG_AND_ID` which is not allowed in
# `file(GENERATE ...)`
function(filter_generator_expressions INPUT OUTPUT)
    # See
    # https://gitlab.kitware.com/cmake/cmake/-/blob/v3.22.2/Modules/FindMPI.cmake#L1218
    ...
    string(REGEX REPLACE "${PROJECT_BINARY_DIR}.*hwloc/src/include.*" "" TMP
                         "${TMP}"
    )
    set(${OUTPUT}
        "${TMP}"
        ...
        if(_GKO_${_gko_interface_lib_hash}_FOUND)
            ...
            if(TARGET ${_lib})
                if("${_lib}" MATCHES "ginkgo.*")
                    ...
                    list(APPEND GINKGO_INTERFACE_LIBS_FOUND
                         "${GINKGO_INTERFACE_LIB_NAME}"
                    )
                endif()
                # Get the link flags and treat them
                get_target_property(
                    GINKGO_INTERFACE_LIBS_LINK_FLAGS "${_lib}"
                ...
                if(_lib_imported AND NOT ${_lib_type} STREQUAL
                                     "INTERFACE_LIBRARY"
                )
                    get_target_property(
                        GINKGO_LIBS_IMPORTED_LIBS "${_lib}"
                ...
                get_target_property(
                    GINKGO_LIBS_INTERFACE_INCS "${_lib}"
                    INTERFACE_INCLUDE_DIRECTORIES
                )
                foreach(_incs ${GINKGO_LIBS_INTERFACE_INCS})
                    filter_generator_expressions(
                        "${_incs}" GINKGO_INTERFACE_INC_FILTERED
                    )
                    if(GINKGO_INTERFACE_INC_FILTERED
                       AND NOT "-I${GINKGO_INTERFACE_INC_FILTERED}" IN_LIST
                ...
                get_target_property(
                    GINKGO_LIBS_INTERFACE_DEFS "${_lib}"
                ...
                get_target_property(
                    GINKGO_LIBS_INTERFACE_OPTS "${_lib}"
                ...
                get_target_property(
                    GINKGO_LIBS_INTERFACE_LIBS "${_lib}"
                    INTERFACE_LINK_LIBRARIES
                )
                # removing $<LINK_ONLY:>
                list(TRANSFORM GINKGO_LIBS_INTERFACE_LIBS
                     REPLACE "\\$<LINK_ONLY:(.*)>" "\\1"
                )
...
macro(ginkgo_pkg_information)
    ...
    get_target_property(
        GINKGO_INTERFACE_LINK_LIBRARIES ginkgo INTERFACE_LINK_LIBRARIES
    )
