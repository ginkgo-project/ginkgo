    ...
    for cycle in v; do
        for mg_mode in ${MG_MODE}; do
            for sm_mode in ${SM_MODE}; do
                ...
                echo "matrix, final residual, iteration count, generation time[ms], total execution time[ms], executation time per iteration[ms], total execution median time[ms]" > ${output}
                for list in AMGX_LISTS MFEM_LISTS; do
                    for matrix in ${!list}; do
                        ...
                        for mixed_mode in 0 1 2 3 7 8 -11 -12 -13 -21 -22 -23 -32 -33 -42 -43; do
                            ...
                            file="${RESULT_FOLDER}/${label}${suffix}_scale0.txt"
                            # delete the line with CRAYBLAS_WARNING from frontier
                            residual_norm=$(sed '/CRAYBLAS_WARNING/d' ${file} | tail -n 7 | head -n 1)
                            info=$(sed '/CRAYBLAS_WARNING/d' ${file} | tail -n 6 | head -n 5 | sed -E 's/[^0-9\.]//g' | tr '\n' ',' | sed -E 's/,$/\n/g')
