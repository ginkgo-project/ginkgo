 ...
 * (roughly) the shape
 * `[global_size_1d / dims[0]; global_size_1d / dims[1] ;
                              ...
                              bool restricted)
{
    ...
    // The width of the discretization. As homogeneous Dirichlet boundary
    // conditions are used, the points on the boundary are implicit, thus
    // the distance between two discretization points is 1/(N + 1).
    const auto h = gko::one<ValueType>() /
                   static_cast<ValueType>(global_discretization_points + 1);
