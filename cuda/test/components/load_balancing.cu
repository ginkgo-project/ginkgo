...
class LoadBalancing : public CudaTestFixture {
protected:
};
...
__global__ void test_1_work_per_chunk(int* output, int* size_once, bool* err,
                                      int size)
{
    gko::kernels::cuda::load_balance_subwarp<
        gko::kernels::cuda::config::warp_size>(
        size,
        [&](int chunk) {
...
TEST_F(LoadBalancing, WorksWith1WorkPerChunk)
{
    for (const auto size :
        ...
        gko::kernels::cuda::components::fill_seq_array(
            exec, ref_data.get_data(), usize);

        test_1_work_per_chunk<<<1, gko::kernels::cuda::config::warp_size>>>(
...
{
    for (const auto size : {0, 1, 2, 3, 10, 11, 19, 20, 21, 30, 31, 32, 33, 34,
                            63, 64, 65, 100}) {
        SCOPED_TRACE(size);
        const auto usize = static_cast<gko::size_type>(size);
        ...
        gko::kernels::cuda::components::fill_seq_array(
            exec, ref_data.get_data(), 3 * usize);

        test_3_work_per_chunk<<<1, gko::kernels::cuda::config::warp_size>>>(
            data.get_data(), size_once.get_data(), err.get_data(), size);
...
TEST_F(LoadBalancing, WorksWithRandomWorkPerChunkNonempty)
{
    std::default_random_engine rng{123468};
    std::uniform_int_distribution<int> dist{1, 100};
    for (const auto size :
         {0, 1, 2, 3, 7, 8, 31, 32, 33, 34, 63, 64, 65, 1000}) {
        SCOPED_TRACE(size);
        const auto usize = static_cast<gko::size_type>(size);
