...
void run1d_reduction(std::shared_ptr<gko::CudaExecutor> exec)
{
    ...
    // 2 * sum i=0...99999 (i+1)
    ASSERT_EQ(output.get_access()[0], 10000100000LL);
    ...
    // 2 * sum i=0...99 (i+1)
    ASSERT_EQ(output.get_access()[0], 10100LL);
}
...
void run2d_reduction(std::shared_ptr<gko::CudaExecutor> exec)
{
    ...
    ASSERT_EQ(output.get_access()[0], 10110100000LL);
    ...
    ASSERT_EQ(output.get_access()[0], 12100LL);
}
